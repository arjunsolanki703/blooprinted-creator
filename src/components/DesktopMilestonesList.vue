  ...
  <div class="milestone-screen-style">
    ...
    <div class="list-margin">
      ...
      <q-list
        ...
        v-for="(milestone, index) in blooprintMileStoneList"
        :key="milestone.id"
        :class="
          index + 1 == blooprintMileStoneList.length ? null : 'milestone-bottom'
        "
      >
        <q-expansion-item
          :ref="milestone.id"
            ...
            <q-item-section side class="milestone-item-side">
              <div class="row items-center gt-xs">
                ...
                >
                  ...
                  <div class="f-w-500 text-black text-body1">
                    {{ milestone.attributes.tasks.length === 0 ? '0 day' : '' }}
                    {{
                    ...
                    {{
                      getTotalTaskHours(milestone) > 0 &&
                      retrieveTotalTaskMinutes(milestone) > 0
                        ? ':'
                        : ''
                    }}
                    {{
              ...
              >
                ...
                >
                  <q-btn
                    ...
                    "
                    :style="task['completed'] ? 'background: #039855' : null"
                    no-caps
...
import TaskDetailsViewPopup from './TaskDetailsViewPopup.vue'
...
export default {
  ...
  methods: {
    ...
    getTotalTaskHours(milestone) {
      ...
      let totalHour = totalHours + Math.floor(totalMinutes / 60)
      let seconds = Number(totalHour * 60 * 60)
    ...
    retrieveTotalTaskMinutes(milestone) {
      const min = this.getTotalTaskMinutes(milestone) % 60
      return min
    },
    ...
    getMinutes(milestone, value) {
      ...
      const m =
        typeof milestone === 'number'
          ? milestone
          : this.retrieveTotalTaskMinutes(milestone)
      if (h > 0 && m > 0) {
        return m < 10 ? '0' + m : m
      } else {
        return m
    ...
    getLabel(number, value) {
      ...
      const m =
        typeof number === 'number' ? number : this.retrieveTotalTaskMinutes(number)
      if (value === 'min') {
        return m < 2 ? ' min' : ' mins'
      } else {
        return h < 2 ? ' hr' : ' hrs'
