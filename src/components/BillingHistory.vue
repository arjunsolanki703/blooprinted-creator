<template>
  <div class="full-width order-history-page">
    <label class="order-history-heading text-letter-space text-capitalize">
    ...
    <div
      ...
      v-if="
        billingHistory != undefined && billingHistory.length > 0 && !isLoading
      "
    >
      <q-markup-table
        flat
      ...
      >
        ...
        <tbody class="full-width" style="background: #F9FAFB !important;">
          <tr v-for="order in billingHistory" :key="order.id">
            ...
            <td class="text-left text-capitalize amount-date-text">
              {{ getDate(order.attributes.startDate) }}
            </td>
            <td class="text-left text-capitalize">
              <label class="status-text" v-if="order.attributes.paid">
                <q-img
        ...
        <q-btn
          v-if="
            billingHistory != undefined &&
              billingHistory.length < billingHistoryCount &&
              !isLoading &&
              !showSpinner
          "
          color="primary"
...
export default {
  ...
  methods: {
    ...mapActions(['getBillingHistory', 'getBillingInvoice']),
    getDate(date) {
      return moment(date).format('ll')
    },
    async downloadInvoice(productID) {
      const { data } = await this.getBillingInvoice(productID)
      axios({
    ...
    async loadMore() {
      ...
      await this.getBillingHistory({
        offset: this.billingHistory.length,
        limit: 10,
        type: 'loadMore'
      })
  ...
  async created() {
    this.isLoading = true
    await this.getBillingHistory({ type: 'get', offset: 0, limit: 10 })
    this.isLoading = false
  },
  computed: {
    ...mapGetters(['billingHistory', 'billingHistoryCount'])
  }
}
</script>
