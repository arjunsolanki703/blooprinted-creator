<template>
  <div>
    <div :style="$q.screen.width > 1024 ? 'margin-bottom:120px' : ''">
      <div
      ...
      >
        <div class="swiper-wrapper">
          <div
            class="swiper-slide"
            v-for="(blooprint, index) in activeBlooprint"
            :key="index"
          >
            <BlooprintDataCard
...
import NoPublishedBlooprints from './NoPublishedBlooprints.vue'
import BlooprintDataCardSkeleton from './BlooprintDataCardSkeleton.vue'
export default {
  name: 'PublishedBlooprints',
  props: ['activeBlooprint', 'publishedCount', 'isLoading'],
  components: { BlooprintDataCard },
  data() {
    return {
      swiperOption: {
        slidesPerView: 1,
  ...
  methods: {
    ...
    unpublishBlooprintPopup(id) {
      this.$emit('unpublish-blooprint', id)
    }
  }
}
</script>
