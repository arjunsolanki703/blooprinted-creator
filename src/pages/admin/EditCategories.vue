...
export default {
  ...
  async created() {
    await this.getParentCategoriesList().then(() => {
    ...
    this.image = this.category.attributes.image
    this.weight = this.category.attributes.weight
    this.favourite = this.category.attributes.favourite
    this.parent = this.parentCategories.find(
      parent => parent.value === this.category.attributes.parent
    )
  },
  ...
  computed: {
    ...mapGetters(['category', 'fileUrl', 'parentCategoriesList'])
  },
  watch: {
    fileUrl(data) {
      this.image = data['url']
