<template>
  <q-page
    ...
    :class="
      $q.screen.lt.md && getWindowHeight > 1000 ? 'setting-page-height' : null
    "
  >
    <div class="settings-page">
      <div class="settings text-letter-space">
        Settings
      </div>
      <div class="flex no-wrap settings-wrapper">
        <div class="left-container" style="">
          <q-card class="menu-card border-radius-lg flex column bg-white">
            ...
            <div
              class="flex no-wrap full-width"
              :class="
                $q.screen.width < 375 ||
                ($q.screen.width > 1023 && $q.screen.width < 1440)
                  ? 'column'
                  : null
              "
            >
              <div
                class="following q-mr-md"
                :style="
                  $q.screen.width < 375 ||
                  ($q.screen.width > 1023 && $q.screen.width < 1440)
                    ? 'width:100%'
              ...
              >
                <label class="following-text">
                  <span
                    ...
                    v-if="userProfile.followers > -1"
                  >
                    {{ profileFollowersCount(userProfile.followers) }}
                  </span>
                  <span v-else
                    ><q-skeleton
                      type="text"
              ...
              >
                <label class="following-text">
                  <span
                    ...
                    >{{ profileFollowersCount(userProfile.following) }}</span
                  >
                  <span v-else
                    ><q-skeleton
                      type="text"
            ...
            >
              ...
              >
                <img
                  src="../../assets/dollar-sign-blue.svg"
                  class="menu-icon"
                  v-if="tab === 'payments'"
                />
                <img
                ...
                >
                  <div
                    class="gt-sm q-py-sm"
                    @click="onClick('payments', false)"
                  >
                    Payments
                  </div>
                  <div
                    class="lt-md q-py-sm full-width"
                    @click="onClick('payments', true)"
                  >
                    Payments
                  </div>
                </label>
              ...
              >
                <img
                  ...
                  v-if="tab === 'myPlan'"
                />
                <img src="../../assets/tag.svg" class="menu-icon" v-else />
                <label
                  class="menu-option-text cursor-pointer text-regalBlue full-width"
                >
                  <div class="gt-sm q-py-sm" @click="onClick('myPlan', false)">
                    My Plan
                  </div>
                  <div
                    class="lt-md q-py-sm full-width"
                    @click="onClick('myPlan', true)"
                  >
                    My Plan
                  </div>
                </label>
        ...
        <div class="full-width right-container" v-if="$q.screen.gt.sm">
          ...
          <q-card class="right-card" v-if="tab === 'myPlan'">
            <my-plan />
          </q-card>
          ...
          <q-card class="right-card" v-if="tab === 'billing'">
            <billing-information @upgrade="val => (tab = val)" />
          </q-card>
          <q-card class="right-card" v-if="tab === 'billing'">
            <billing-history />
          </q-card>
...
<script>
import EditProfilePicture from 'src/components/EditProfilePicture.vue'
import { mapGetters } from 'vuex'
import Account from 'src/components/Account.vue'
import DeleteAccount from 'src/components/DeleteAccount.vue'
import SettingsContent from 'src/components/SettingsContent.vue'
import MyPlan from 'src/components/MyPlan.vue'
import CreatorProfile from 'src/components/CreatorProfile.vue'
import OrderHistory from 'src/components/OrderHistory.vue'
import BillingHistory from 'src/components/BillingHistory.vue'
import BillingInformation from 'src/components/BillingInformation.vue'
import Notifications from 'src/components/Notifications.vue'
import BankAccountDetails from 'src/components/BankAccountDetails.vue'
import DownloadCustomerData from 'src/components/DownloadCustomerData.vue'
export default {
  components: {
    DownloadCustomerData,
    EditProfilePicture,
  ...
  mounted() {
    this.$root.$on('myPlanComponent', () => {
      this.onClick('myPlan', true)
    });
  },
  data() {
    return {
