<template>
  <q-page class="bg-whiteSmoke">
    ...
    <div class="q-mx-xl q-mt-sm text-nightRider">
      Select blooprints to make it featured
    </div>
    <div
...
export default {
  ...
  mounted() {
    ...
    this.getFeaturedlooprints()
  },

  methods: {
    ...mapActions([
      'retrieveBlooprintList',
      'addBlooprintsToFeatured',
      'fetchFeaturedBlooprintsList',
      'setToastMessage'
    ]),
    ...
    async saveBlooprintsToPopular() {
      ...
      await this.addBlooprintsToFeatured(params)
    },
    // ************* Method to retrieve Popular Blooprints List **********************
    async getFeaturedlooprints() {
      let response = await this.fetchFeaturedBlooprintsList()
